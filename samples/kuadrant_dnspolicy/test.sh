...
kubectl apply -f scratch/kuadrant_dnspolicy/managed-cluster-set_gateway-clusters.yaml
kubectl apply -f scratch/kuadrant_dnspolicy/managed-cluster-set-binding_gateway-clusters.yaml
kubectl apply -f scratch/kuadrant_dnspolicy/placement_http-gateway.yaml
kubectl create -f hack/ocm/gatewayclass.yaml

# Create gateway (myapp.mn.hcpapps.net)
kubectl apply -f scratch/kuadrant_dnspolicy/gateway_prod-web.yaml
...
kubectl --context kind-mgc-control-plane apply -f scratch/kuadrant_dnspolicy/echo-app.yaml
kubectl --context kind-mgc-workload-1 apply -f scratch/kuadrant_dnspolicy/echo-app.yaml
kubectl --context kind-mgc-workload-2 apply -f scratch/kuadrant_dnspolicy/echo-app.yaml
...
kubectl --context kind-mgc-control-plane get dnsrecord myapp.mn.hcpapps.net -n multi-cluster-gateways -o json | jq .metadata.ownerReferences
kubectl --context kind-mgc-control-plane get managedzone -n multi-cluster-gateways

## Check new endpoint structure
kubectl --context kind-mgc-control-plane get dnsrecord myapp.mn.hcpapps.net -n multi-cluster-gateways -o json | jq .spec.endpoints
